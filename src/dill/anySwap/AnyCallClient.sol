...
interface ISidechainGaugeProxy {
    function sendRewards(uint256 periodId, uint256 amount, int256[] memory weights) external;
}
...
contract AnyswapTokenAnycallClient is AnycallClientBase {
    ...
    event LogSwapout(
        ...
        uint256 amount, uint256 toChainId, int256[] weights
    );
    event LogSwapin(
        address indexed token, address indexed sender, address indexed receiver,
        uint256 amount, uint256 fromChainId, int256[] weights
    );
    ...
    modifier onlyDistributor() {
        require(msg.sender == executor, "AnycallClient: onlyDistributor");
        _;
    }
    ...
    function bridge(
        ...
        emit LogSwapout(token, msg.sender, receiver, amount, toChainId, weights);
    }
        ...
        returns (bool success, bytes memory result)
    {
        ...
        if (selector == this.anyExecute.selector) {
            (
                ...
                data[4:],
                (address, address, uint256, address, address, uint256, int256[], uint256)
            );
            ...
            if (_underlying == address(0) || (IERC20(dstToken).balanceOf(address(this)) < amount)) {
                assert(IAnyswapToken(dstToken).mint(address(this), amount));
            }
        ...
        whenNotPaused(PAUSE_FALLBACK_ROLE)
    {
        ...
        (
            ...
            data,
            (bytes4, address, address, uint256, address, address, uint256, int256[])
        );
