...
describe("Vote & Distribute : chunk and onlyGov distribution", () => {
  before("Setting up gaugeProxyV2", async () => {
    ...
    GaugeMiddleware = await upgrades.deployProxy(
      gaugeMiddleware,
  ...
  it("change gauge gaugeProxy in middleware and add gauges successfully", async () => {
    await expect(
      GaugeMiddleware.connect(userSigner).changeGaugeProxy(GaugeProxyV2.address)
    ).to.be.revertedWith("can only be called by governance");
    await GaugeMiddleware.changeGaugeProxy(GaugeProxyV2.address);

    /** add gaugeMiddleware*/
    console.log("-- Adding Gauge middleWare --");

    await expect(GaugeProxyV2.addGaugeMiddleware(zeroAddr)).to.be.revertedWith(
      "gaugeMiddleware cannot set to zero"
    );
    await GaugeProxyV2.addGaugeMiddleware(GaugeMiddleware.address);
    await expect(
      GaugeProxyV2.addGaugeMiddleware(GaugeMiddleware.address)
    ).to.be.revertedWith("current and new gaugeMiddleware are same");

    await expect(GaugeProxyV2.addGauge(zeroAddr)).to.be.revertedWith(
      "address of token cannot be zero"
    );

    await expect(
      GaugeMiddleware.addGauge(
        zeroAddr,
        governanceAddr,
        ["PICKLE"],
        [pickleAddr]
